...
namespace Asteroid_Run
{
    public class Game1 : Game
    {
        ...
        {
            MainMenu, ShipSelection, Instructions, Intro, Playing, GameOver
        }
        ...
        protected override void LoadContent()
        {
            ...
            _phantomTexture = Content.Load<Texture2D>("Sprites/phantom");
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            switch (_currentState)
            {
                ...
                case GameState.ShipSelection:
                    ...
                    if (clickedShip != -1)
                    {
                        _currentShipIndex = clickedShip;
                        _player.SetTexture(_shipTextures[_currentShipIndex]);
                    }
        ...
        private void UpdateGameplay(GameTime gameTime, float deltaTime)
        {
            ...
            float currentEnemySpeedMult = 1.0f;
            if (_slowdownTimer > 0)
            {
                _slowdownTimer -= deltaTime;
                currentEnemySpeedMult = 0.4f;
            }
            ...
            if (_distance >= 1500f && _currentZone < 3) { _currentZone = 3; _backgroundManager.ChangeTexture(_bgPurple); }
            else if (_distance >= 500f && _currentZone < 2) { _currentZone = 2; _backgroundManager.ChangeTexture(_bgDarkPurple); }
            ...
            if (_meteorTimer > spawnDelay)
            {
                ...
                float speed = _random.Next(150, 300) * difficultyMultiplier;
                _meteors.Add(new Meteorite(randomTex, pos, speed, (float)(_random.NextDouble() * 2 - 1)));
                _meteorTimer = 0;
            }
            ...
            {
                Vector2 pos = new Vector2(_random.Next(100, 700), -50);
                _ufos.Add(new Ufo(_ufoTexture, pos, 120f * difficultyMultiplier));
                _ufoTimer = 0;
            }
            ...
            {
                _meteors[i].Update(enemyTime);
                if (_meteors[i].Position.Y > 650) { _meteors.RemoveAt(i); i--; }
            }

            for (int i = 0; i < _ufos.Count; i++)
            {
                _ufos[i].Update(enemyTime);
                if (_ufos[i].Position.Y > 650) { _ufos.RemoveAt(i); i--; }
            }

            for (int i = 0; i < _phantoms.Count; i++)
            {
                _phantoms[i].Update(enemyTime, _player.Position, _projectiles);
                if (_phantoms[i].Position.Y > 650) { _phantoms.RemoveAt(i); i--; }
            }

            for (int i = 0; i < _projectiles.Count; i++)
            {
                if (_projectiles[i].IsEnemy) _projectiles[i].Update(enemyTime);
                else _projectiles[i].Update(gameTime);
        ...
        private void DrawGameplay()
        {
            ...
            string zoneText = $"ЗОНА: {_currentZone}";

            _spriteBatch.DrawString(_loreFont, scoreText, new Vector2(22, 22), Color.Black);
            _spriteBatch.DrawString(_loreFont, scoreText, new Vector2(20, 20), Color.Cyan);
            _spriteBatch.DrawString(_loreFont, distanceText, new Vector2(22, 52), Color.Black);
            _spriteBatch.DrawString(_loreFont, distanceText, new Vector2(20, 50), Color.Orange);
            _spriteBatch.DrawString(_loreFont, zoneText, new Vector2(22, 82), Color.Black);
            _spriteBatch.DrawString(_loreFont, zoneText, new Vector2(20, 80), Color.Red);
            ...
            if (_slowdownTimer > 0)
            {
                _spriteBatch.DrawString(_loreFont, $"ЗАМЕДЛЕНИЕ: {(int)_slowdownTimer + 1} СЕК", new Vector2(20, yOffset), Color.LimeGreen);
            }
        }
    }
}
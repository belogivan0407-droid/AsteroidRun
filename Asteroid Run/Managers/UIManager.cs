...
namespace Asteroid_Run
{
    internal class UIManager
    {
        ...
        public struct ShipButton
        {
            ...
            public ShipButton(Rectangle bounds, Texture2D texture)
            {
                Bounds = bounds;
                Texture = texture;
                CurrentColor = Color.White;
        ...
        {
            Point mousePos = new Point(_currentMouse.X, _currentMouse.Y);
            if (_backButton.Bounds.Contains(mousePos))
            {
                _backButton.CurrentColor = Color.Orange;
                return IsClicked();
            }
            _backButton.CurrentColor = Color.White;
            return false;
        }
        ...
        private int ProcessTextButtons(List<MenuButton> buttons)
        {
            ...
            for (int i = 0; i < buttons.Count; i++)
            {
                ...
                {
                    button.CurrentColor = Color.White;
                }
                buttons[i] = button;
            }
        ...
        {
            spriteBatch.Draw(_bgTexture, new Rectangle(0, 0, 800, 600), Color.White);
            DrawCenteredText(spriteBatch, "ИНСТРУКЦИЯ", 80, Color.Cyan);

            string instructions =
                "ЦЕЛЬ ИГРЫ:\n" +
                "Пролететь как можно дальше сквозь аномалию и выжить.\n" +
                "Сбивайте вражеские объекты для набора очков.\n\n" +
                "УПРАВЛЕНИЕ:\n" +
                "[A] / [СТРЕЛКА ВЛЕВО] - Движение влево\n" +
                "[D] / [СТРЕЛКА ВПРАВО] - Движение вправо\n" +
                "[ПРОБЕЛ] - Огонь лазером\n" +
                "[F11] - Полный экран\n" +
                "[ESC] - Выход из игры";

            spriteBatch.DrawString(_font, instructions, new Vector2(120, 160), Color.White);
